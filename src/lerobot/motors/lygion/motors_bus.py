...
class SerialMotorsBus(MotorsBusBase):
    ...
    def set_half_turn_homings(
        ...
        for motor, offset in homing_offsets.items():
            if self.motors[motor].model != "stepper":
    ...
    def set_init_position(self, id_input, position_input):
        com, scs_error = self.packet_handler.reOfsCal(id_input, position_input)
        if not self._is_comm_success(com):
            print("%s" % self.packet_handler.getTxRxResult(com))
        else:
    ...
    def _unnormalize(self, ids_values: dict[int, float]) -> dict[int, int]:
        ...
        for id_, val in ids_values.items():
            ...
            if max_ == min_:
                raise ValueError(f"Invalid calibration for motor '{motor}': min and max are equal.")
            if self.motors[motor].norm_mode == MotorNormMode.RANGE_M100_100:
                val = -val if drive_mode else val
                bounded_val = min(100.0, max(-100.0, val))
                unnormalized_values[id_] = int(((bounded_val + 100) / 200) * (max_ - min_) + min_)
            elif self.motors[motor].norm_mode == MotorNormMode.RANGE_0_100:
                val = 100 - val if drive_mode else val
                bounded_val = min(100.0, max(0.0, val))
                unnormalized_values[id_] = int((bounded_val / 100) * (max_ - min_) + min_)
            elif self.motors[motor].norm_mode == MotorNormMode.DEGREES:
                mid = (min_ + max_) / 2
                max_res = self.model_resolution_table[self._id_to_model(id_)] - 1
                unnormalized_values[id_] = int((val * max_res / 360) + mid)
            elif self.motors[motor].norm_mode == MotorNormMode.STEPPER:
                max_res = self.model_resolution_table[self._id_to_model(id_)] - 1
                if motor == "shoulder_pan":
                    mid = max_res/2
                    unnormalized_values[id_] = int(-(val * max_res / 360) + mid)
    ...
    def _setup_sync_reader(self, motor_ids: list[int], addr: int, length: int) -> None:
        ...
        for id_ in motor_ids[3:6]:
            self.sync_reader.addParam(id_,MOTOR_TYPE_SERVO)

        for id_ in motor_ids[0:3]:
            self.sync_reader.addParam(id_,MOTOR_TYPE_STEPPER)
            # self.sync_reader.addParam(id_,MOTOR_TYPE_SERVO)
    ...
    def sync_write(
        ...
        *,
        ...
        Args:
            ...
            num_retry (int, optional): Retry attempts.  Defaults to `0`.
        """
        values["elbow_flex"] = values["elbow_flex"]+values["shoulder_lift"]
        ...
        if data_name == "Goal_Position":
            for id_,val in int_ids_values.items():
                if self._id_to_model(id_) == "stepper":
                    self.packet_handler.SyncWritePosEx(
                        id_,val, STEPPER_SPEED,STEPPER_ACC,STEPPER_CURRENT
                    )
                elif self._id_to_model(id_) == "sts3215":
                    self.packet_handler.SyncWritePosEx(
                        id_,val, SERVO_SPEED,SERVO_ACC,SERVO_CURRENT
                    )  
            scs_comm_result = self.packet_handler.groupSyncWrite.txPacket()

            if not self._is_comm_success(scs_comm_result):
