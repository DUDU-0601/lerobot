...
def _split_into_byte_chunks(value: int, length: int) -> list[int]:
    from . import lydevs_sdk as ly

    if length == 1:
        data = [value]
    elif length == 2:
        data = [ly.SCS_LOBYTE(value), ly.SCS_HIBYTE(value)]
    elif length == 4:
        data = [
            ly.SCS_LOBYTE(ly.SCS_LOWORD(value)),
            ly.SCS_HIBYTE(ly.SCS_LOWORD(value)),
            ly.SCS_LOBYTE(ly.SCS_HIWORD(value)),
            ly.SCS_HIBYTE(ly.SCS_HIWORD(value)),
        ]
    return data
...
class LygionMotorsBus(SerialMotorsBus):
    """
    The Lygion MotorsBus class allows to efficiently read and write to the attached motors. It relies on the
    python lygion sdk to communicate with the motors, which is itself based on the dynamixel sdk.
    """
    ...
    def __init__(
        ...
        self.port_handler = ly.PortHandler(self.port)
        # HACK: monkeypatch
        self.port_handler.setPacketTimeout = patch_setPacketTimeout.__get__(  # type: ignore[method-assign]
            self.port_handler, ly.PortHandler
        )
        self.packet_handler = ly.TTLSDClass(self.port_handler)
        self.sync_reader = ly.GroupSyncRead(self.packet_handler, 0, 0)
        self.sync_writer = ly.GroupSyncWrite(self.packet_handler, 0, 0)
        self._comm_success = ly.COMM_SUCCESS
        self._no_error = 0x00
    ...
    def _broadcast_ping(self) -> tuple[dict[int, int], int]:
        ...
        rx_length = 0
        wait_length = status_length * ly.MAX_ID

        txpacket = [0] * 6

        tx_time_per_byte = (1000.0 / self.port_handler.getBaudRate()) * 10.0

        txpacket[ly.PKT_ID] = ly.BROADCAST_ID
        txpacket[ly.PKT_LENGTH] = 2
        txpacket[ly.PKT_INSTRUCTION] = ly.INST_PING

        result = self.packet_handler.txPacket(self.port_handler, txpacket)
        if result != ly.COMM_SUCCESS:
            self.port_handler.is_using = False
            return data_list, result

        # set rx timeout
        self.port_handler.setPacketTimeoutMillis((wait_length * tx_time_per_byte) + (3.0 * ly.MAX_ID) + 16.0)
        ...
        if rx_length == 0:
            return data_list, ly.COMM_RX_TIMEOUT

        while True:
            if rx_length < status_length:
                return data_list, ly.COMM_RX_CORRUPT
            ...
            if idx == 0:  # found at the beginning of the packet
                ...
                if rxpacket[status_length - 1] == checksum:
                    result = ly.COMM_SUCCESS
                    data_list[rxpacket[ly.PKT_ID]] = rxpacket[ly.PKT_ERROR]
