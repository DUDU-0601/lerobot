...
from lerobot.motors.lygion.motors_bus import MotorNormMode
from lerobot.motors.lygion import (
    LygionMotorsBus,
    OperatingMode,
)
...
from .config_aloha_mini_follower import AlohaMiniFollowerRobotConfig

logger = logging.getLogger(__name__)


class AlohaMiniFollower(Robot):
    """
    Generic aloha mini follower base implementing common functionality.
    Designed to be subclassed with a per-hardware-model `config_class` and `name`.
    """

    config_class = AlohaMiniFollowerRobotConfig
    name = "aloha_mini_follower"

    def __init__(self, config: AlohaMiniFollowerRobotConfig):
        super().__init__(config)
        self.config = config
        # choose normalization mode depending on config if available
        norm_mode_body = MotorNormMode.DEGREES
        self.bus = LygionMotorsBus(
            port=self.config.port,
            motors={
                "shoulder_pan": Motor(7, "stepper", MotorNormMode.STEPPER),
                "shoulder_lift": Motor(8, "stepper", MotorNormMode.STEPPER),
                "elbow_flex": Motor(9, "stepper", MotorNormMode.STEPPER),
...
AlohaMiniFollower = AlohaMiniFollower
